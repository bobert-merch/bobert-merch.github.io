...
- *No Add to Cart button* — the add-on card has no cart button at all. Clicking a `.swatch-btn` both selects the design (visually) and, if eligible, adds/swaps it in the cart directly — see the swatch click handler in site.js. There's no automatic retry: if you pick a design while ineligible, add a sticker, then click the *same* swatch again to actually add it.
- *Gating* — it can only be in the cart alongside a real paid item. `hasQualifyingItem()` checks for any cart line with `price > 0` and `productId !== 'discord-banner'`. `enforceAddonEligibility()` strips the banner out if that's false — called on every `saveCart()` and once at load (to catch a cart saved before this rule existed). `updateAddonAvailability()` just toggles `.addon-hint`'s visibility while ineligible (there's no button to disable). Clicking a swatch while ineligible only updates the visual selection/preview — it does not touch the cart.
